...
async function getAlbumTitle(value)
{
    ...
    let response = await fetch(`${PATH}/api/album/${value}/search`);
...
{
    let response = await fetch(`${PATH}/api/artist/${value}/description`);
    let content = await response.json();
...
{
    let response = await fetch(`${PATH}/api/${albumValue}/${artistValue}/cover`);
    let content = await response.json();
